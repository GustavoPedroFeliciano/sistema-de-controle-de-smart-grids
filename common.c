...
enum MSG_TYPE {INS_REQ = 1, REM_REQ, CH_REQ, SEN_REQ, VAL_REQ, SEN_RES, VAL_RES, ERROR, OK};
unsigned parse_msg_type(const char *msg_type_in){
    if(!strcmp(msg_type_in, "INS_REQ"))
        ...
        return CH_REQ;
    else if (!strcmp(msg_type_in, "SEN_REQ"))
        return SEN_REQ;
    else if (!strcmp(msg_type_in, "VAL_REQ"))
        return VAL_REQ;
    else if (!strcmp(msg_type_in, "SEN_RES"))
        return SEN_RES;
    else if (!strcmp(msg_type_in, "VAL_RES"))
        return VAL_RES;
